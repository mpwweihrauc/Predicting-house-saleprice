...
```{r}
dataset[which(is.na(dataset$Exterior1st)), ]$Exterior1st
dataset[which(is.na(dataset$Exterior2nd)), ]$Exterior2nd
```
...
We plot "BsmtFinType1" against "SalePrice" and observe that houses with unfinished or good quality living quarter type of finish come with higher "SalePrice".
...
dataset %>%
  ...
  theme_bw() +
  ggtitle("GarageYrBlt vs. YearBlt")
```
...
As a final step we fit the model with the above determined parameters on the entire training subset `train`. Accordingly, `train` has to be prepared via "one-hot-encoding". We can then fit the final "xgbTree" model with the hyperparameters from the final tuning round. We are using 3-times repeated 5-fold cross-validation in this final step.
...
```{r, error = FALSE, message = FALSE, warning = FALSE, fig.height = 3, fig.width = 6}
...
Finally, we can predict on `test` and prepare the submission file for entry into the **Kaggle** competition, where the submission is evaluated and scored on a public leaderboard. At the time of this writing, the generated submission file scored an RMSE of 0.12588 and achieved a rank of 1471 on the public leaderboard of the respective competition. The public leaderboard is calculated with approximately 50% of the test data and the final results will be based on the other 50%, so final scores are subject to change. Curiously, the actual public score at **Kaggle** is very close to the RMSE determined by cross-validation during the final model fitting above (RMSE of `min(xgb_final_model$results$RMSE)`).
