...
From the tuning plots and the selected best parameters we can see that relatively shallow trees of `max_depth` `r xgb_1st_tuning$bestTune$max_depth` were chosen, along with a moderate `min_child_weight` of `r xgb_1st_tuning$bestTune$min_child_weight`. After some further tuning of other hyperparameters, we will see whether a slower learning rate `eta` would improve things.
...
In the third and last round of hyperparameter tuning we will determine the optimal learning rate `eta` and the optimal number of boosting iterations `nrounds`. The slower the learning rate, the more "fine-grained" the fitting process will be. The algorithm learns in smaller steps, but can therefore find finer optima. `eta` achieves this by shrinking the determined variable weights afer each step. It is therefore also called "step size shrinkage". By shrinking feature weights each step, the algorithm becomes more conservative. The plot of the final model clearly shows that a lower learning rate `eta` can reach a lower RMSE value with increasing iterations, as is the case for `eta` of 0.025. At an `eta` of 0.1 we can see that learning is already concluded after about 500 boosting iterations, however the achieved RMSE is a little bit higher.
