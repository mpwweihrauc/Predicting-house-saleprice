...
## Summary of Data Analysis and Manipulations
...
**Please note that model fitting is utilizing parallelization, which might require more RAM and cause increased CPU heat production during modelling.**
...
From the tuning plots and the selected best parameters we can see that relatively shallow trees of `max_depth` 3 were chosen, along with a moderate `min_child_weight` of 4. The number of boosting iterations to reach the lowest RMSE with these settings is also a moderate 550. After some further tuning of other hyperparameters, we will see whether a slower learning rate `eta` would improve things.
...
We can now predict on `test_set` and record "out-of-bag" RMSE. We note a small improvement over the first tuning round.
...
In the third and last round of hyperparameter tuning we will determine the optimal learning rate `eta` and the optimal number of boosting iterations `nrounds`. The slower the learning rate, the more "fine-grained" the fitting process will be. The algorithm learns in smaller steps, but can therefore find finer optima. `eta` achieves this by shrinking the determined variable weights afer each step. It is therefore also called "step size shrinkage". By shrinking feature weights each step, the algorithm becomes more conservative.
...
We can now predict on `test_set` and record "out-of-bag" RMSE. We note a small improvement over the second tuning round.
...
As a final step we fit the model with the above determined parameters on the entire training subset `train`. Accordingly, `train` has to be prepared via "one-hot-encoding". We can then fit the final "xgbTree" model with the hyperparameters from the final tuning round.
...
We can then visualize the final model and take a look at the lowest cross-validation RMSE value, prior to predicting against `test` (which doesn't contain a "SalePrice" column). The plot of the final model clearly shows that a lower learning rate `eta` can reach a lower RMSE value with increasing iterations, as is the case for `eta` of 0.025. At an `eta` of 0.1 we can see that learning is already concluded after about 500 boosting iterations, however the achieved RMSE is a little bit higher. 
...
Finally, we can predict on `test` and prepare the submission file for entry into the **Kaggle** competition, where the submission is evaluated and scored on a public leaderboard. At the time of this writing, the generated submission file scored an RMSE of 0.12313 and achieved a rank of 1248 on the public leaderboard of the respective competition. The public leaderboard is calculated with approximately 50% of the test data and the final results will be based on the other 50%, so final scores are subject to change. Curiously, the actual public score at **Kaggle** is very close to the RMSE determined by cross-validation during the final model fitting above (RMSE of `r min(xgb_final_model$results$RMSE)`).
